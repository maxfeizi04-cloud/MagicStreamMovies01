...
测试环境: 本地 Windows，50 并发连接，10 管道因子，持续 10 秒

┌─────────────┬───────────────┬──────────┬──────────┬──────────┐
│     端点     │ QPS (请求/秒)  │  平均延迟  │  P99 延迟 │ 最大延迟  │
├─────────────┼───────────────┼──────────┼──────────┼──────────┤
│ GET /health │ 96,736        │ 4.69ms   │ 14ms     │ 39ms     │
├─────────────┼───────────────┼──────────┼──────────┼──────────┤
│ GET /movies │ 13,623        │ 36ms     │ 53ms     │ 70ms     │
├─────────────┼───────────────┼──────────┼──────────┼──────────┤
│ GET /genres │ 16,904        │ 29ms     │ 43ms     │ 62ms     │
├─────────────┼───────────────┼──────────┼──────────┼──────────┤
│ POST /login │ 18,348        │ 27ms     │ 41ms     │ 54ms     │
└─────────────┴───────────────┴──────────┴──────────┴──────────┘
...
│     端点     │  QPS   │  平均延迟  │ P99 延迟 │
├─────────────┼────────┼──────────┼──────────┤
│ GET /movies │ 14,956 │ 66ms     │ 108ms    │
└─────────────┴────────┴──────────┴──────────┘

结论:
- 服务端表现良好，无报错、无超时
- /health 接近 10 万 QPS，纯内存响应
- /movies 数据库查询约 1.3-1.5 万 QPS，延迟稳定
- 100 并发下吞吐量基本持平，延迟翻倍但仍可控
- 登录接口全部返回 401（测试用了错误密码），属正常行为，说明密码校验逻辑正确
